...
export default function ConfidenceIndicator({ gesture, confidence, isDetecting }: ConfidenceIndicatorProps) {
  const pct = Math.round((confidence || 0) * 100)
  return (
    <div className="glass-card p-4">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2 text-sm text-gray-400"><RiBarChartLine size={16} /><span>Confidence</span></div>
      ...
      {gesture ? (
        <div className="space-y-2">
          ...
          <div className="confidence-bar">
            <motion.div className={`confidence-fill bg-gradient-to-r ${confidenceColor(confidence)}`}
        ...
        <div className="space-y-2">
          ...
          <div className="confidence-bar"><div className="confidence-fill w-0" /></div>
        </div>
      )}
    </div>
  )
