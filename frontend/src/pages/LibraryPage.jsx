...
const BUILT_IN_GESTURES = [
  { name: 'Hello', count: 20, builtin: true },
  { name: 'Help', count: 20, builtin: true },
  { name: 'Water', count: 20, builtin: true },
  { name: 'Thank You', count: 20, builtin: true },
  { name: 'Please', count: 20, builtin: true },
  { name: 'Yes / Good', count: 20, builtin: true },
  { name: 'No / Bad', count: 20, builtin: true },
  { name: 'Stop', count: 20, builtin: true },
  { name: 'I Love You', count: 20, builtin: true },
  { name: 'Peace / 2', count: 20, builtin: true },
  { name: 'Point / 1', count: 20, builtin: true },
  { name: 'L', count: 20, builtin: true },
  { name: 'Y', count: 20, builtin: true },
  { name: 'A', count: 20, builtin: true },
  { name: '4', count: 20, builtin: true },
]
...
export default function LibraryPage() {
  ...
  const filteredBuiltin = BUILT_IN_GESTURES.filter(g =>
    g.name.toLowerCase().includes(search.toLowerCase())
  )

  const filteredCustom = customGestures.filter(g =>
    g.name.toLowerCase().includes(search.toLowerCase())
  )
      ...
      <div className="max-w-5xl mx-auto">
            ...
            <div>
              <div className="flex items-center gap-3 mb-1">
                <div className="w-9 h-9 rounded-xl bg-purple-600/20 flex items-center justify-center">
                  <RiBookLine size={18} className="text-purple-400" />
                </div>
        ...
        {tab === 'builtin' ? (
          ...
          customGestures.length === 0 && !search ? (
            <div className="glass-card p-12 text-center">
              ...
              <p className="text-gray-600 text-sm mb-6">Train your first gesture in the AI Training Dashboard</p>
              <Link to="/train">
        ...
        <div className="mt-8 glass-card p-4 flex items-center justify-between">
          <span className="text-sm text-gray-500">
            {tab === 'builtin' ? filteredBuiltin.length : filteredCustom.length} gesture{(tab === 'builtin' ? filteredBuiltin.length : filteredCustom.length) !== 1 ? 's' : ''} shown
          </span>
          <span className="text-xs text-gray-600">
            {BUILT_IN_GESTURES.length} built-in · {customGestures.length} custom
          </span>
        </div>
      </div>
    </div>
