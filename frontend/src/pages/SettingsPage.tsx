  ...
  return (
    <div className="glass-card p-6">
      <div className="flex items-center gap-3 mb-5">
        <div className="w-8 h-8 rounded-lg bg-purple-600/20 flex items-center justify-center"><Icon size={16} className="text-purple-400" /></div>
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    const loadVoices = () => setVoices(speechService?.getVoices() ?? [])
    loadVoices()
      ...
      <div className="max-w-3xl mx-auto">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <div className="flex items-center gap-3 mb-1">
            <div className="w-9 h-9 rounded-xl bg-purple-600/20 flex items-center justify-center"><RiSettings3Line size={18} className="text-purple-400" /></div>
        ...
        <div className="space-y-5">
              ...
              <div className="flex gap-3">
                {[{ value: 'dark', icon: RiMoonLine, label: 'Dark' }, { value: 'light', icon: RiSunLine, label: 'Light' }].map(({ value, icon: Icon, label }) => (
                  <button key={value} onClick={() => updateSetting('theme', value as AppSettings['theme'])}
                    className={`flex items-center gap-2 px-4 py-2 rounded-xl border text-sm font-medium transition-all ${settings.theme === value ? 'bg-purple-600/20 border-purple-500/40 text-purple-300' : 'bg-white/5 border-white/10 text-gray-400 hover:text-gray-200'}`}>
                    <Icon size={16} />{label}
                  </button>
                ))}
              </div>
              ...
              <div className="flex gap-3">
                {(['small', 'medium', 'large'] as const).map(size => (
                  <button key={size} onClick={() => updateSetting('subtitleSize', size)}
                    className={`px-4 py-2 rounded-xl border text-sm font-medium capitalize transition-all ${settings.subtitleSize === size ? 'bg-purple-600/20 border-purple-500/40 text-purple-300' : 'bg-white/5 border-white/10 text-gray-400 hover:text-gray-200'}`}>
                    {size}
                  </button>
                ))}
              </div>
          ...
          <SettingSection title="Voice & Speech" icon={RiVolumeUpLine}>
            ...
            <button id="test-voice-btn" onClick={testVoice} className="btn-outline w-full flex items-center justify-center gap-2"><RiVolumeUpLine size={18} />Test Voice</button>
          </SettingSection>
          ...
          <SettingSection title="AI Recognition" icon={RiCpuLine}>
            ...
            <p className="text-xs text-gray-600">Higher = fewer false positives. Lower = more sensitive. Recommended: 75%</p>
          </SettingSection>
